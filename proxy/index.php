...
if (isset($_GET["q"]) && strpos($_GET["q"], "mbasic.facebook") !== false){
header("Origins: https://mbasic.facebook.com");
header("Origin: https://mbasic.facebook.com");
} else if (isset($_GET["q"]) && strpos($_GET["q"], "instagram.com") !== false){
header("Origins: https://www.instagram.com");
...
if(isset($_POST['url'])){
	...
	$url = $_POST['url'];
	$url = add_http($url);
	$proxify = proxify_url($url);
	//die($proxify);
	header("HTTP/1.1 302 Found");
	header('Location: '. $proxify);
	exit;
	
} else if(!isset($_GET['q'])){
  header("Location: /".basename(__DIR__).'/'.basename(__FILE__)."?q=https://mbasic.facebook.com/?_fb_noscript=1");
  die();
