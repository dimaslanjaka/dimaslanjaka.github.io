/* skel-layout.js v3.0.2-dev | (c) skel.io | MIT licensed */
    ...
    applyGridTransforms: function () {
      ...
      _.iterate(cells, function (i) {
        ...
        if (!Object.prototype.hasOwnProperty.call(cell, key) || cell[key] === false) {
          ...
          // Responsive level?
          if (
            cell.className.match(/important\((.+)\)/) &&
            (l = parseInt(__.gridLevelMap.v[RegExp.$1])) <= config.grid.level
          )
            mode = 'l';
      ...
      if (__.config.conditionals) {
        ...
        if (!(x = _.attachment(id))) {
          ...
          _.iterate(_.obj.breakpoints, function (k) {
            if (_.indexOf(_.breakpointIds, k) !== -1) s1.push('.not-' + k);
            else s2.push('.only-' + k);
          });
